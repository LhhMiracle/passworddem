...
async function logout() {
  ...
  chrome.tabs.query({}, (tabs) => {
    tabs.forEach(tab => {
      chrome.tabs.sendMessage(tab.id, { type: 'LOGGED_OUT' }).catch(() => {});
    });
  });
}
...
async function updateBadge(count) {
  ...
  await chrome.action.setBadgeBackgroundColor({ color: count > 0 ? '#0052CC' : '#666' });
}
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === 'fill-password') {
    ...
    chrome.tabs.sendMessage(tab.id, { type: 'SHOW_PASSWORD_PICKER' });
  } else if (info.menuItemId === 'generate-password') {
    // 生成密码并填充
    const password = generatePassword();
    chrome.tabs.sendMessage(tab.id, { type: 'FILL_GENERATED_PASSWORD', password });
...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === 'fill_password') {
    ...
    if (tab) {
      chrome.tabs.sendMessage(tab.id, { type: 'QUICK_FILL' });
    }
  }
});

console.log('Password Vault Extension Service Worker started');
